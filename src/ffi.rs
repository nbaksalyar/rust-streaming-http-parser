...
pub type HttpCallback = extern "C" fn(*mut HttpParser) -> libc::c_int;
pub type HttpDataCallback = extern "C" fn(*mut HttpParser, *const u32, libc::size_t) -> libc::c_int;
...
impl HttpParser {
    pub fn new(parser_type: ParserType) -> HttpParser {
        ...
        unsafe {
            http_parser_init(&mut p as *mut _, parser_type);
        }
        return p;
    }
    ...
    pub fn http_body_is_final(&self) -> libc::c_int {
        unsafe {
            return http_body_is_final(self);
        }
    }
